...
function EditProduct() {
  ...
  useEffect(() => {
    async function getUser() {
      await fetch("http://localhost:3003/sofalight/backend/api/getLoggedIn", {
        credentials: "include",
      })
        .then((d) => d.json())
        .then((d) => {
          ...
          if (
            ...
            d.user === "Couldn't get user data"
          ) {
            console.log(d, "not logged in");
            window.location = "/";
            return;
          }
          document.getElementById("preloaderScreenEditProduct").style.display =
            "none";
        })
        .catch((e) => {
          console.log("error");
  ...
  let [productName, setProductName] = useState(
    productInfos.productInfo.productName
  );
  let [productSubText, setProductSubText] = useState(
    productInfos.productInfo.productSubText
  );
  let [productCategory, setProductCategory] = useState(
    productInfos.productInfo.productCategory
  );
  let [productPrice, setProductPrice] = useState(
    productInfos.productInfo.productPrice
  );
  let [productDiscount, setProductDiscount] = useState(
    productInfos.productInfo.productDiscount
  );
  let [productStatus, setProductStatus] = useState(
    productInfos.productInfo.productStatus
  );
  let [productDescription, setProductDescription] = useState(
    productInfos.productInfo.productDescription
  );
  ...
  const failureToast = (
    <CToast>
      ...
      <CToastBody>
        Product not updated. Refresh the page and try again or contact support
      ...
      <div className="sofa_light_dashboard_furniturestore_AddProduct">
        <>
          ...
          <CButton
            ...
            onClick={() => addToast(emptyerrorbuttonId)}
          >
            Send a toast
          </CButton>
        ...
        <EditActionButton
          handleSave={async (e) => {
            ...
            let saveStatus = await saveProduct(
              productInfos._id,
              products,
              textInfo
            );
