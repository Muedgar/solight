...
function ViewProductsAdmin(props) {
  ...
  useEffect(() => {
    async function getUser() {
      let url = "http://localhost:3003/sofalight/backend/api/getLoggedIn"
      await fetch(url, {
        credentials: "include",
      })
        .then((d) => d.json())
  ...
  useEffect(() => {
    function handleResize() {
      if (window.innerWidth > 1263) {
        document.querySelector(".products_show").style.gridTemplateColumns =
          "1fr 1fr 1fr";
      }
      if (window.innerWidth < 1263) {
        document.querySelector(".products_show").style.gridTemplateColumns =
  ...
  useEffect(() => {
    async function getData() {
      let url = "http://localhost:3003/sofalight/backend/api/products"
      if(props.name) {
        if(props.name==="Mattress") {
          url = "http://localhost:3003/sofalight/backend/api/mattress"
        }
      }
    ...
    if (activeSort === "Highest Price") {
      for (let i = 0; i < inData.length; i++) {
        for (let j = i + 1; j < inData.length; j++) {
          if (
            Number(inData[i].productInfo.productPrice) <
            Number(inData[j].productInfo.productPrice)
          ) {
            let temp = inData[i];
            inData[i] = inData[j];
            inData[j] = temp;
      ...
      <div className="sofa_light_dashboard_furniturestore_components_viewproducts">
        ...
        >
          <div className="sofa_light_dashboard_furniturestore_components_viewproducts_filter_title">
            ...
            <p style={{ marginTop: "20px" }}>Filter</p>
          </div>
          ...
          <div className="sofa_light_dashboard_furniturestore_components_pricerange">
            <p style={{ paddingTop: "20px" }} className="sizeOne">
              Price Range
            </p>
            <input
              className="slider"
              type="range"
              value={filterValPrice}
              onChange={(e) => {
        ...
        <div>
          ...
          <div className="sofa_light_dashboard_furniturestore_components_sortby">
            ...
            <p>
              <span
                onClick={(e) => setActiveSort("Highest Price")}
                className={
              ...
              <span
                onClick={(e) => setActiveSort("Lowest Price")}
                className={
                ...
                sortDataBy(data).map((d, k) => {
                  if (Number(d.productInfo.productPrice) < filterValPrice) {
                    ...
                    if (d.productInfo.productCategory) {
                      if (
                        filterValCategory.toLowerCase() === "all" ||
                        d.productInfo.productCategory.toLowerCase() ===
                          filterValCategory.toLowerCase()
                      ) {
                        if (
                          filterSearchVal.trim() === "" ||
                          d.productInfo.productName
                            .toLowerCase()
                            .includes(filterSearchVal.toLowerCase().trim())
                        ) {
                          return (
                            <ProductAdmin
                              productData={d}
