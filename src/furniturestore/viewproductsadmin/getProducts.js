...
const GetProducts = (props) => {
  ...
  useEffect(() => {
    async function getData() {
      if ("all" === props.filter.category) {
        // instead of handling each filter item, take all parameters into consideration
      ...
      {data.length > 0 ? (
        data.map((d, k) => (
          <ProductAdmin
            ...
            productprice={formatPrice(d.productInfo.productPrice)}
          />
        ))
      ) : (
