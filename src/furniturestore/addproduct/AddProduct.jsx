...
import saveMattress from "./saveMatress";

function AddProduct(props) {
  // let checkLogin = 'check'
  //   useEffect(() => {
  //       async function getUser() {
  //           await fetch("http://localhost:3003/sofalight/backend/api/getLoggedIn",{credentials: "include"})
  //           .then(d => d.json())
  //           .then(d => {
  //               console.log(d);
  ...
  const emptyerrorbuttonIdmattress = (
    <CToast>
      <CToastHeader closeButton>
        <svg
          className="rounded me-2"
      ...
      <CToastBody>
        Some of products information are empty and you need to upload at least
        one image
      </CToastBody>
    </CToast>
  );
    ...
    <>
      ...
      <div className="sofa_light_dashboard_furniturestore_AddProduct">
        <>
          <CButton
            style={{ display: "none" }}
            id="successbuttonIdmattress"
            onClick={() => addToast(successToast)}
          >
            Send a toast
          </CButton>
          ...
          <CButton
            style={{ display: "none" }}
            id="failurebuttonIdmattress"
            onClick={() => addToast(failureToast)}
          >
            Send a toast
          </CButton>
        ...
        <div
          ...
          className="sofa_light_dashboard_furniturestore_components_imageUpload"
        >
          <img src={upload} alt="upload" />
          <p>
            <span>Click and Browse</span> to Choose a File
          </p>
        </div>
        ...
        {/* save product */}
        <ActionButton
          buttonId={props.type !== "furniture"?"saveProductButtonIdmattress":"saveProductButtonId"}
          id={props.type !== "furniture"?"saveProductStatusButtonIdmattress":"saveProductStatusButtonId"}
          handleSave={async (e) => {
            if (
              productName === "" ||
              productSubText === "" ||
              ...
              document.getElementById("emptyerrorbuttonIdmattress").click();
              return;
            }
            let textInfo = {
              productName,
            ...
            document.getElementById("saveProductStatusButtonIdmattress").style.display =
              "block";
            document
              .getElementById("saveProductButtonIdmattress")
              .setAttribute("disabled", "true");
            /// how to send data to the back end.
            let saveStatus = null;
            if (props.type === "furniture") {
              saveStatus = await saveProduct(products, textInfo);
            } else {
              saveStatus = await saveMattress(products, textInfo);
            ...
            if (saveStatus === "saved") {
              ...
              document.getElementById(
                "saveProductStatusButtonIdmattress"
              ).style.display = "none";
              document.getElementById("successbuttonIdmattress").click();
              document
                .getElementById("saveProductButtonIdmattress")
                .removeAttribute("disabled");
            } else {
              document.getElementById("failurebuttonIdmattress").click();
            }
          }}
        />
      </div>
